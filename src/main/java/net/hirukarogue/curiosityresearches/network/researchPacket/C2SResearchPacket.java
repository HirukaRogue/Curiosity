package net.hirukarogue.curiosityresearches.network.researchPacket;

import net.hirukarogue.curiosityresearches.events.NetworkEvents;
...
public class C2SResearchPacket {
    private final BlockPos blockPos;

    public C2SResearchPacket(BlockPos blockPos) {
        this.blockPos = blockPos;
    }

    public C2SResearchPacket(FriendlyByteBuf buffer) {
    ...
    public void handle(Supplier<NetworkEvent.Context> contextSupplier) {
        ...
        context.enqueueWork(() -> {
            ...
            if (player.level().getBlockEntity(blockPos) instanceof ResearchTableBlockEntity researchTable) {
                String ponderMessage = researchTable.research();
