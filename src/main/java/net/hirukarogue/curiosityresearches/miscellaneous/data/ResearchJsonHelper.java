...
public class ResearchJsonHelper {
    private static final Codec<Map<String, Component>> COMPONENTS_CODEC = Codec.unboundedMap(Codec.STRING, Component.CODEC);

    public static NonNullList<Component> deserialize(JsonObject pSerializedRecipe) {
        JsonObject desserializedJson = GsonHelper.getAsJsonObject(pSerializedRecipe, "keys");
        Map<String,Component> componentReference = COMPONENTS_CODEC.parse(JsonOps.INSTANCE, desserializedJson).resultOrPartial(k -> {
            CuriosityMod.LOGGER.error("Failed to parse component key for research recipe: " + k);
        }).orElse(Map.of());

        JsonArray desserializedJson2 = GsonHelper.getAsJsonArray(pSerializedRecipe, "pattern");
        List<String> pattern = Codec.STRING.listOf().parse(JsonOps.COMPRESSED,desserializedJson2).result().orElse(List.of());
